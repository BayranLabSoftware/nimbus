...
/**
 * Rule 1188 (d): H5's engine — a fork of the sealed candidate
 * (`fcmBranch.ts`, rule 1162, untouched) carrying NO physics change. Every
 * equation, every branch of every break, every line of the ledger is copied
 * unchanged; the only additions are a generation counter and the recording,
 * at the moment a piece is born or lands, of state the branch's own step
 * already holds (birth altitude, speed, mass, inherited strength, material
 * density, and its mass at half the altitude it drops after birth) — never
 * a new physical quantity, only a longer memory of one the sealed engine
 * already computes and discards. Because nothing about the dynamics
 * changes, this file's own tests hold it to EXACT equality with the sealed
 * candidate's output, not merely close, on a spread of random cases.
 *
...
export interface FcmH5Piece {
   ...
   *  without structure. */
  group: number;
  /** Rule 1188 (a): 0 for the unbroken body landing whole, 1 for a piece
   *  born at the first break, one more than its parent's otherwise. */
  generation: number;
  birthAltitudeM: number;
  birthSpeedMS: number;
  birthMassKg: number;
  inheritedStrengthPa: number;
  /** The material density its area follows, from birth (rule 1188 (a)). */
  rhoKgM3: number;
  /** Mass at birth, at half the altitude dropped since birth (linearly
   *  interpolated between the two nearest simulated steps), and at landing
   *  (equal to `mass`): rule 1188 (a)'s per-interval mass-loss trace. */
  massTraceKg: readonly [number, number, number];
}

export interface FcmH5Result {
  completed: boolean;
  components: number;
  mass: number;
  energy: number;
...
interface Component {
  cloud: boolean;
  /** Rule 1188 (a): 0 for the entry body, one more than its parent's for
   *  anything born at a break — the only field this fork adds to flight
   *  state; everything else H5 records (birth h/v/mass/strength/rho) is
   *  already sitting unchanged on this same object throughout its flight. */
  generation: number;
...
export function fcmEntryH5(body: FcmBody, options: FcmOptions): FcmH5Result {
  ...
  const fly = (c: Component): void => {
    ...
    let h = c.h;
    // Rule 1188 (a): this component's own mass at half the altitude it
    // drops between its birth (c.h) and the ground — only meaningful (and
    // only used) for a piece that lands as itself, never for one that
    // breaks again first.
    const midAltitudeM = c.h / 2;
    let midMassKg: number | null = null;
    ...
    for (;;) {
      ...
      record(h, y, hNext, next, c.n);
      if (midMassKg === null && h > midAltitudeM && hNext <= midAltitudeM) {
        const frac = (h - midAltitudeM) / (h - hNext);
        midMassKg = y[2] + frac * (next[2] - y[2]);
      }
      y = next;
      h = hNext;
      if (y[2] < floor) {
      ...
      if (h <= 0) {
        ...
        if (c.cloud) {
          ...
          pieces.push({
            ...
            rhoKgM3: c.rho,
            massTraceKg: [c.mass, midMassKg ?? y[2], y[2]],
          });
        }
        return;
      }
...
// fcmProfile/fcmPeaks are not forked here: H5 needs no peak, only the
// landed pieces' own history; a run script wanting them imports the sealed
// candidate's own `fcmPeaks` (fcmBranch.js), which takes any object shaped
// like its `FcmResult` — this fork's result satisfies that shape.
