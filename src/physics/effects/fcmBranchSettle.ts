...
/**
 * Rule 1227 (src/physics/validation/porta1Rules.ts): a fork of the sealed
 * `fcmBranch.ts` — round 3's frozen candidate (rule 1162) is untouched by this
 * file, and this file does not touch it. The one change: the condition that
 * stops a cloud at its terminal speed says what rule 1138 (c) says. The
 * sealed text stops a cloud whenever its speed is below 1.01 × its local
 * terminal speed, which a fresh cloud high in thin air has before it has
 * slowed at all (the first draw of Tunguska: 69.6 % of the mass stopped at
 * birth at 39.9 km, 15.0 km/s against a local terminal speed of 22.5 km/s).
 * Here a cloud settles once its speed has been above `settleWithin` over its
 * terminal speed in its own flight and has since come down within that band
 * — it has slowed to its terminal speed; a cloud still below the band flies
 * on, whether the thin air cannot yet hold it up or the denser air is only
 * now bringing its terminal speed down to its own. Everything else is
 * the same code as fcmBranch.ts, copied rather than imported so that a later
 * change to the sealed file cannot silently reach this one; where no cloud is
 * ever born below its terminal speed the two agree to the last bit
 * (`fcmBranchSettle.test.ts`).
 */
/**
...
export function fcmEntrySettle(body: FcmBody, options: FcmOptions): FcmResult {
  ...
  const fly = (c: Component): void => {
    ...
    // Rule 1227: whether this cloud's speed has been above the settling band
    // in its own flight.
    let aboveBand = false;
    for (;;) {
      const here = (): Component => ({ ...c, v: y[0], gamma: y[1], mass: y[2], radius: y[3], h });
      // Rule 1138 (c), as rule 1227 reads it: a cloud settles once its speed
      // has been above `settleWithin` over its terminal speed in its own
      // flight and has since come down within that band — it has slowed to
      // its terminal speed. Still below the band — a terminal speed the thin
      // air cannot yet reach, or one the denser air is only now bringing
      // down to its speed — it flies on.
      if (c.cloud && withGravity) {
        if (y[0] > settleAbove * cloudTerminal(h, y)) aboveBand = true;
        else if (aboveBand) {
