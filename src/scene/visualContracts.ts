...
export const VISUAL_CONTRACTS = {
  ...
  mmiField: defineContract({
    ...
    caveats: [
      ...
      'the ground-motion residual is sigma_lnY = 0.60 about the median, which through Worden et al. 2012 is about ONE WHOLE INTENSITY DEGREE: a contour labelled VII could honestly be VI or VIII',
      'that degree is DRAWN since 20 September 2026: each band is painted as three contours, at one sigma below its level, at half a sigma below, and at the level itself, with rising opacity, so the colour is solid where the level is certain and fades outward across the sigma where it is not. The median contour keeps its line and its numeral, so the level the report quotes is still a line a reader can point at. Only outward: an equal fan on both sides washed every band into its neighbour and cost the map its four colours',
  ...
  impactFieldMap: defineContract({
    ...
    caveats: [
      ...
      'below the lowest threshold the field is not coloured: an overpressure or a wind under 0.5 psi, a fluence under the lowest burn or fire drawn, a blanket thinner than 1 mm, a shaking below V (as the earthquakes are drawn; III is a line); past the top of a scale the colour saturates',
      "past a field's edge the globe draws what the model says there (rules 1028 to 1036): where the field goes on below its threshold, one neutral veil fading out to a decade below the edge's value — a unit of effective magnitude for the shaking — or to the model's limit, labelled 'continues below threshold'; where the model stops (the fireball's direct horizon), a thick dash-dot line, 'limit of the model', never an isoline; past it, a grey hatch, labelled with what is not computed. Each carries its provenance card in the legend and the report; none reads a number the layer does not give. The ejecta's band says 'deposit < 1 mm or not resolved at the current scale', never that the blanket continues; I3's agreement draws none. A heat whose every drawn threshold is zero while the model still gives first-degree burns keeps its tab, drawn as the same veil in a halo out to that reach, 'below the main threshold' (rule 1031 (d)). Out of the crater's domain every ring is concentric about the source, the whole field lightly hatched and labelled 'exploratory effects: ground coupling not modelled', every card out of domain (rule 1031 (e))",
      ...
      "the low overpressure (rule 1031 (c)) is the overpressure's own field read from 0.2 kPa up to 0.5 psi, where the structural layer starts, on a scale of its own, lines at 0.2, 0.7, 1 and 3 kPa, each saying what the sources give at it (rule 1246 (c): Gi et al. 2018's light window damage, about one pane in twenty after Mannan & Lees, no named meaning, a quarter to more than half of the panes): exploratory — not validated as a prediction of the field — and outside the structural scale and the toll",
      'an airburst is drawn round: the trajectory’s own shock, which laid Tunguska’s forest down in a butterfly, stands only in I3’s band (a ceiling while that band holds the measured footprints); every complete airburst’s blast layers say so in a note (rule 1246 (d)): a shallow entry draws the region out along the track, and beneath it the overpressure can exceed the drawn one beyond the band’s factor two, as in Chelyabinsk’s city',
    ],
  }),
