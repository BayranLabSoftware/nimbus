...
airburst to a partial one; and the entry's two jumps, B-089 (the program's
doubled I_f against the paper's) and B-091 (the paper's own I_f = 1, a body
that lands whole beside one that bursts), which the own seed does not draw
but an unseen one does, with three burn rings, one far wave and an 11 m
