...
| B-016 | D-FORM           | Vaiont confined-basin factor 3 tuned on a "250 m wave" that is the slide's thickness                  | n/a (landslide preset)                                                                  | `B-016 Vaiont confined-basin factor set on the wave its source gives`              | pending                                                           |
| B-017 | D-FORM + D-IMPL  | Chemical surface burst blasted at its yield in a free-air fit and drew nuclear flash rings            | `pnpm simulate --event=explosion --preset=BEIRUT_2020`                                  | `B-017 A chemical charge on the ground blasts at twice its yield and has no flash` | pending                                                           |
| B-018 | D-FORM           | Nuclear crater coefficients twice Glasstone & Dolan's dry-soil crater                                 | `pnpm simulate --event=explosion --preset=ONE_MEGATON`                                  | `B-018 Nuclear crater coefficients follow Glasstone & Dolan §6.09 for dry soil`    | pending                                                           |
| B-019 | D-FORM           | Nuclear surface bursts used the air-burst thermal partition 0.35                                      | `pnpm simulate --event=explosion --preset=CASTLE_BRAVO_1954`                            | `B-019 A nuclear surface burst radiates with the 0.18 partition`                   | pending                                                           |
