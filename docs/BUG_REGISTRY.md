...
| B-011 | D-FORM           | Impact "Teanby-Wookey" Mw read 10⁻⁴·E as a seismic moment: ≈ 2.9 units low     | `pnpm simulate --event=impact --preset=CHICXULUB`                                       | `B-011 Impact seismic magnitude is Collins Eq. 40 on the ground-coupled energy` | pending                                                           |
| B-012 | D-IMPL           | Ejecta thickness computed with the final rim radius, 2.4–10× too thick         | `pnpm simulate --event=impact --preset=METEOR_CRATER`                                   | `B-012 Ejecta thickness uses the transient crater diameter`                     | pending                                                           |
| B-013 | D-FORM           | Complex crater depth from a lunar fit: jump 627 → 1 482 m at 3.2 km            | `pnpm simulate --event=impact --preset=CHICXULUB`                                       | `B-013 Complex crater depth follows Collins Eq. 28, not a lunar fit`            | pending                                                           |
