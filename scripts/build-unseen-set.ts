 ...
 *
 * Usage:
 *   pnpm exec tsx scripts/build-unseen-set.ts [cache directory]
 *
 * Every response is kept in the cache directory (default: the system's
 * temporary directory), so a run that ComCat throttles can be run again
 * without asking twice for what it already has.
 *
...
/** ComCat answers 429 to four at a time over a thousand events. */
const CONCURRENCY = 2;
...
/** An answer no retry will change: ComCat's 404 and 409, an event it no
 *  longer serves or has merged into another. */
...
async function json(url: string): Promise<unknown> {
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      // Throttled or dropped: back off, and longer each time.
...
): Promise<{ row: UnseenEarthquake } | { missing: string } | { gone: string }> {
  let d: Detail;
  try {
 ...
 *  them. */
export const UNSEEN_UNREADABLE: readonly string[] = [
${gone.map((id) => `  ${JSON.stringify(id)},`).join('\n')}
];
`;
  const out = join(
  ...
  console.error(
    `listed ${listed.length.toString()}, unseen ${unseen.length.toString()}: wrote ${sorted.length.toString()} (${sorted.filter((r) => r.ncei.length === 0).length.toString()} quiet), ${missing.length.toString()} without coverage, ${gone.length.toString()} unreadable; ${(bytes / 1e6).toFixed(1)} MB read → ${out}`
  );
}
