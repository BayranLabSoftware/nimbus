/**
 * Rules 1180 (b), 1187 (src/physics/validation/fcmSurvivalLightRules.ts): H4
 * — every solid component's σ fixed at rule 1131's own named sensitivity,
 * 3.5·10⁻⁷ s²/m² (Borovička et al. 2013b), against the baseline of H3's
 * fixed-σ runs (fcmSurvivalLight.json) — on the same 18 development cases,
 * the same input and parameter streams (rule 1179): only the ablation
 * representation differs; a body's diameter, speed, density, angle,
 * strength, α, split and cloud dispersion are the exact draws H3 used. A
 * copy of scripts/fcm-h2-run.ts: the engine (fcmBranchH2.ts) is unchanged,
 * only σ's range is degenerate instead of drawn from rule 1131's prior.
 * Writes src/physics/validation/fcmH4Run.json and docs/FCM_H4_RUN.md.
 *
 *   pnpm exec tsx scripts/fcm-h4-run.ts all
 *   pnpm exec tsx scripts/fcm-h4-run.ts shard k K
 *   pnpm exec tsx scripts/fcm-h4-run.ts merge K
 *
...
import { FCM_H4_SIGMA_S2_M2 } from '../src/physics/validation/fcmSurvivalLightRules.js';
...
const SHARDS_DIR = join(tmpdir(), 'nimbus-fcm-h4-run');
 ...
 *  stream is passed for σ, as H2's fork requires, but H4's degenerate range
 *  never calls it: nothing is drawn, only rule 1187's fixed value returned. */
function drawH4(
  ...
  const options: FcmH2Options = {
    sigmaRange: [FCM_H4_SIGMA_S2_M2, FCM_H4_SIGMA_S2_M2],
    sigmaDraw,
...
function flyCase(spec: {
  ...
  const configurations = CONFIGURATIONS.map(({ structure, cloud }) => {
    ...
    const rows = spec.inputs.map((x, i) => {
      const sigmaU = stream(`fcm-h4-sigma/${spec.name}/${structure}/${cloud}/${String(i)}`);
      const d = drawH4(entryOf(x), structure, cloud, u, sigmaU);
...
function merge(K: number): void {
  ...
  writeFileSync('src/physics/validation/fcmH4Run.json', `${JSON.stringify(out, null, 1)}\n`);
  ...
  const lines = [
    ...
    '',
    `**Landed mass, H4 against H3, ${String(compared.length)} case–configuration pairs**: median ratio ${String(medianRatio)}`,
  ...
  writeFileSync('docs/FCM_H4_RUN.md', lines.join('\n'));
  console.log(
    JSON.stringify(
