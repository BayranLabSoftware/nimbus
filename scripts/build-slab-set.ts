...
import { ATLAS_SEEN } from '../src/physics/validation/atlasRules.js';
...
import type { SlabEarthquake } from '../src/physics/validation/slabRules.js';
import { SMALL_DEEP_EARTHQUAKES } from '../src/physics/validation/smallDeepSetData.js';
...
/**
 * The earthquakes of rule 66 of `src/physics/validation/slabRules.ts`.
 *
 * Every M 6 or larger event of 1973 to 2025, deeper than 70 km and no deeper
 * than 300 km, that USGS ComCat holds a ShakeMap for, less the earthquakes
 * the project had already read: its preferred origin, magnitude and moment tensor (rules 1 to 3), its
 ...
 * subduction-interface models (rule 35) and to its subduction-slab models,
 * how the map was drawn (stations,
 ...
 *
 * Usage:
 *   pnpm exec tsx scripts/build-slab-set.ts [cache directory]
 *
...
const LIST = `${COMCAT}?format=geojson&starttime=1973-01-01&endtime=2026-01-01&minmagnitude=6&mindepth=70.0001&maxdepth=300&producttype=shakemap&orderby=time-asc`;
const NCEI =
  'https://www.ngdc.noaa.gov/hazel/hazard-service/api/v1/earthquakes?minYear=1973&maxYear=2025';
const COVERAGE = 'download/coverage_mmi_low_res.covjson';
const INFO = 'download/info.json';
const CONCURRENCY = 2;
const CACHE = resolve(process.argv[2] ?? join(tmpdir(), 'nimbus-slab-set'));
...
async function one(id: string, ncei: readonly NceiItem[]): Promise<Outcome> {
  ...
  let slabWeight = 0;
  const slabSets: string[] = [];
  sets.forEach((g, i) => {
    const name = g.name ?? '';
    if (name.startsWith('subduction_interface')) {
      interfaceWeight += weights[i] ?? 0;
      interfaceSets.push(name);
    } else if (name.startsWith('subduction_slab')) {
      slabWeight += weights[i] ?? 0;
      slabSets.push(name);
    }
  });
...
async function main(): Promise<void> {
  ...
  const rows = new Map<string, SlabEarthquake>();
  ...
  const body = `// Generated by scripts/build-slab-set.ts on ${readOn}. Do not edit by
...
type Line = readonly [
  ...
  number, string, number, number, boolean, string, number, string,
];
...
const quake = ([
  ...
  finiteFault, revision, slabWeight, slabSet,
]: Line): SlabEarthquake => ({
  comcat, time, place, magnitude, magnitudeType, depthKm, latitude, longitude, faultType,
  maxMmi, areaKm2: { 7: a7, 8: a8, 9: a9 }, ncei, deaths, missing,
  interfaceWeight, interfaceSet, stations, reports, finiteFault, revision, slabWeight, slabSet,
});
...
export const SLAB_WITHOUT_COVERAGE: readonly string[] = ${JSON.stringify(missing)};

/** Listed events whose ShakeMap carries no model weights, left out. */
export const SLAB_WITHOUT_MODEL: readonly string[] = ${JSON.stringify(noModel)};

/** Listed events ComCat would not serve that day, left out. */
export const SLAB_UNREADABLE: readonly string[] = ${JSON.stringify(gone)};
`;
  const out = join(
    resolve(dirname(fileURLToPath(import.meta.url)), '..'),
    'src',
