...
/**
 * The round of rules 440 to 447: the geometry and the top band, apart and
 * together, on all three sets in one run.
 *
 * Usage:
 *   pnpm exec tsx scripts/top-band-round.ts
 */
...
function main(): void {
  ...
  for (const arm of ARMS) {
    ...
    console.log(
      `  ${arm.key.padEnd(12)} inside ${inside.toString()}/${inside0.toString()} · ` +
  ...
  console.log('| arm | toll >= 10 | band reaches zero | median band width (ln) |');
  console.log('| --- | --- | --- | --- |');
  for (const arm of ARMS) {
    let loud = 0;
    let zero = 0;
    ...
    console.log(
      `| ${arm.key} | ${loud.toString()} | ${zero.toString()} of ${banded.toString()} | ` +
