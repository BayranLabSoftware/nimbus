...
import type { DeepInterfaceEarthquake } from '../src/physics/validation/interfaceStadiumRules.js';
...
/**
 * The earthquakes of rule 40 of `src/physics/validation/interfaceStadiumRules.ts`.
 *
 * Every M 6 to 7.5 event of 2008 to 2025 deeper than 40 km and no deeper
 * than 70 km that USGS ComCat holds a ShakeMap for, less the events of rule
 * 11's and rule 23's sets: its preferred origin, magnitude and moment
 * tensor (rules 1 to 3), its ShakeMap's ground at MMI VII, VIII and IX
 * (summed as `pnpm shakemap:build` sums it), the weight the map's
 * ground-motion model gave its subduction-interface models (rule 35), and
 * the NCEI records within two minutes and 200 km of it (rule 23). Only
 * those numbers are written. Nothing here runs the simulator.
 *
 * Usage:
 *   pnpm exec tsx scripts/build-deep-interface-set.ts [cache directory]
 *
 * Every response is kept in the cache directory, named as
 * scripts/build-unseen-set.ts names it, whose NCEI pages it shares.
 */

const COMCAT = 'https://earthquake.usgs.gov/fdsnws/event/1/query';
const LIST = `${COMCAT}?format=geojson&starttime=2008-01-01&endtime=2026-01-01&minmagnitude=6&maxmagnitude=7.4999&mindepth=40.0001&maxdepth=70&producttype=shakemap&orderby=time-asc`;
const NCEI =
...
const CONCURRENCY = 2;
const CACHE = resolve(process.argv[2] ?? join(tmpdir(), 'nimbus-unseen-set'));
...
async function main(): Promise<void> {
  ...
  const rows = new Map<string, DeepInterfaceEarthquake>();
  ...
  const body = `// Generated by scripts/build-deep-interface-set.ts on ${readOn}. Do not edit
// by hand: rule 40 of interfaceStadiumRules.ts says what these are and how
// they were read. USGS ComCat and ShakeMap (${COVERAGE}, ${INFO}), public
...
export const DEEP_INTERFACE_READ_ON = '${readOn}';

/** Events rule 40's ComCat query returned on that day, before rule 11's
 *  and rule 23's were taken out. */
export const DEEP_INTERFACE_LISTED = ${listed.length.toString()};
...
const quake = ([
  ...
  interfaceWeight, interfaceSet, stations, reports,
});
...
export const DEEP_INTERFACE_WITHOUT_COVERAGE: readonly string[] = ${JSON.stringify(missing)};

/** Listed events whose ShakeMap carries no model weights, left out. */
export const DEEP_INTERFACE_WITHOUT_MODEL: readonly string[] = ${JSON.stringify(noModel)};

/** Listed events ComCat would not serve that day, left out. */
export const DEEP_INTERFACE_UNREADABLE: readonly string[] = ${JSON.stringify(gone)};
`;
  const out = join(
    ...
    'physics',
    'validation',
    'deepInterfaceSetData.ts'
  );
  writeFileSync(out, body);
  console.error(
    `listed ${listed.length.toString()}, fresh ${fresh.length.toString()}: wrote ${sorted.length.toString()} (${sorted.filter((r) => r.interfaceWeight >= 0.5).length.toString()} interface, ${sorted.filter((r) => r.interfaceWeight >= 0.5 && r.ncei.length === 0).length.toString()} of them quiet), ${missing.length.toString()} without coverage, ${noModel.length.toString()} without model weights, ${gone.length.toString()} unreadable; ${(bytes / 1e6).toFixed(1)} MB read → ${out}`
  );
}
