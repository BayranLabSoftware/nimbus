...
/**
 * The round of rules 427 to 434: the dip from the structure the strike came
 * from.
 *
 * Three arms, one run. Arm B is the best geometry the last round measured
 * and the one it refused; arm C is B with this round's dip, and they differ
 * in nothing else, so C against B is the dip on its own. Nothing here
 * chooses anything — the verdict is read off rule 433's six clauses.
 *
 * Usage:
 *   pnpm exec tsx scripts/dip-round.ts
 */

/** Rule 432's arms. The first is the geometry in place; `dip` says whether
 *  the arm asks `shippedDipAnswer` for the structure's own dip. */
const ARMS = [
  { key: 'in place', settings: {}, dip: false },
  {
...
/** Rule 428: the dip reaches the simulator as an input, from the one
 *  answer, and the rupture length it needs comes from the scenario drawn
 *  without it — which cannot move, because no law reads the dip to size a
 *  rupture. */
function withDip(
...
/** Rule 431: the geometry is decided on the law the product draws. */
...
function scenarioFor(event: AtlasEarthquake, law: ContourLaw, arm: Arm): EarthquakeScenarioInput {
  return withDip(
    {
...
function withSettings(event: RecordedEvent, arm: Arm): RecordedEvent {
  const run = event.run;
  return {
    ...event,
    run: () => {
...
/** Rule 433(d), on every arm. */
function theDead(): void {
  console.log('\n### rule 433(d) — the dead, on the net rows');
  ...
  for (const arm of ARMS) {
    ...
    RECORDED_EVENTS.forEach((row, i) => {
      ...
      const b = compareWithRecord(withSettings(row, arm));
...
function main(): void {
  ...
  if (site === null) {
    console.error('The shipped Vs30 tiles are absent; rule 431 asks for the ground the browser');
  ...
  table(`the jury of ${jury.length.toString()} — rules 427 to 433`, wide);
  ...
  console.log(`\n### rule 433, clause by clause (${DECIDING_LAW})`);
  for (const arm of ARMS) {
    if (arm.key === 'in place') continue;
