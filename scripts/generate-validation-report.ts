...
${bullet([
  ...
  "**The ash cloud is far too narrow across the wind, and the closure that would widen it was refused.** This is the worst number in the project. Nimbus spreads a release by σ_y = max(0.3 · H, 500 m) · √(1 + x / 10H) — the downwind distance and the plume height, and nothing else — so a 32 µm grain that takes a day to reach the ground spreads exactly as much as an 8 mm lapillus that takes four minutes. Against Tephra2, the advection–diffusion model the field runs (Connor & Connor 2006), on the 70 eruptions of the benchmark matrix, the loading is 0.51× on the wind axis and **0.008×** fifty kilometres downwind and thirty across: a hundred and twenty-five times too little, at σ_ln 31.8. Thirty kilometres off the axis the ratio is zero to two decimals. On 16 September 2026 Tephra2's own closure — where the spread a release gets is the one its fall time earns — was implemented, pre-registered as rules 106 to 109 and measured: it gives 0.299× across the wind at σ_ln 4.1 and 0.686× on the axis at σ_ln 1.37, with 59 % of points within a factor of two against 23 %. Every figure improves, and rule 108 refused it anyway, on its other clause: rule 19's invariants came back at 222 where the rule names 221. The 222nd failure is not the closure's — the sweep gives 222 with the candidate switched off — and is not a defect, but the number 221 was read before two of the same day's rounds changed the explosion's physics, and a bound is not moved after a figure has failed it. The closure sits in `events/volcano/ashfall.ts` as `spreadLaw: 'tephra2'`, reachable and unused; what a visitor sees is the narrow cloud. The round, the reference and the refusal are in `docs/BENCHMARK_PROTOCOL.md` and reproducible from `benchmark/results/tephra2-reference-2026-09-16.json`.",
