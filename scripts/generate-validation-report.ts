...
${bullet([
  ...
  "**The burn rings are drawn at a threshold, not at a probability.** Since 16 September 2026 an explosion's burn rings are read off Glasstone & Dolan's Figure 12.64, whose caption is \"Radiant exposure required to produce skin burns for different skin pigmentations\" and which attaches no probability to any curve. The book's next figure, 12.65, gives the same thing as probabilities — solid lines for 50 % of an average exposed population, broken lines for the 18 % and 82 % bands — and this project does not read it. Where the two stand beside each other: at 1 Mt the traced first-degree curve for a middling skin asks 3.32 cal/cm², below the 4.5-to-6 band in which §12.65 puts 18 % of an exposed population at second-degree burns and the rest at first, and the second-degree curve asks 6.30, just above it. 12.65 has since been traced too (`scripts/benchmark/burn-probability-curves.py`), and it reproduces its own worked example on its own figure: at 1 Mt its 18 %-second-degree band runs 4.71 to 6.25 cal/cm² where the book reads 4.5 to 6. Set beside it, 12.64's middle pigmentation asks a little more exposure than 12.65's 50 % line everywhere — 8 % more at 1 kt and 14 % at 10 Mt for a first-degree burn, 2 % and 9 % for a second, 4 % and 7 % for a third — so the rings this product draws are between 1 % and 7 % narrower than 50 % rings would be. Nothing is adopted on that reading: which figure a simulator should draw is a choice about what a ring is meant to mean, and it has not been made. Tracing 12.65 also puts the book's 18 % and 82 % bands within reach, which the product has never had. Rules 80 to 84 described the curves as the exposure half a population takes until the same day; that gloss was wrong and is corrected in their own text, the numbers unchanged. Reading Figure 12.65 instead is a round nobody has run (docs/SCIENCE.md, \"The exposure that burns, from the book's own figure\").",
