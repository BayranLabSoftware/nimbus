...
const CONTOUR_LAW_LABEL: Readonly<Record<ContourLaw, string>> = {
  ...
  campbellBozorgnia2014: 'Campbell & Bozorgnia (2014), NGA-West2, with the hypocentral depth',
...
/** I2 as the scorecard reads it: against the field's tool (rules 126 to 128). */
const I2_MET_AGAINST_THE_FIELD =
  GOLD_STANDARD_SCORECARD.find((d) => d.domain === 'Impacts')?.rules.find((r) => r.rule === 'I2')
    ?.status === 'met';

/**
 * The entry's miss of the sky, declared as a gap until 21 September 2026.
 * It is the field's own — the program misses the same fireballs by as much,
 * and the model is the program on them (I2, met under the amendment of
 * 16 September 2026) — so it is printed in the entry's section beside the
 * program's, and a gap is declared here only if I2 stops being met.
 */
function fireballGap(run: FireballRunResult): string[] {
  const first = FIREBALL_BODIES[0];
  const reading = run.readings[first.key];
  if (reading === undefined || run.meetsBar || I2_MET_AGAINST_THE_FIELD) return [];
...
${bullet([
  ...
  "**An impact's air blast is read from under the ground, as the program reads it.** Since 16 September 2026 the blast of an impact that reaches the ground is the Earth Impact Effects Program's own (rules 138 to 145): its airburst law at the altitude Collins et al.'s Eq. 18 gives, which for such a body lies below the ground, so the crossover of the Mach relation shortens and the blast weakens as a steeper entry puts that altitude deeper. It reproduces the program within 1 % on every point read, and it is what the field's tool computes, not a physical source: no paper derives a burst below the surface. At a fixed range a larger body can blast less, and since 21 September 2026 the invariants ask of every blast ring that shrinks as its body grows whether its source moved it (rules 683 to 690): on the sweep's own seed each of 396 is moved by the altitude of its source, true or below the ground, which moves without a step. Where the program has no answer the simulator chooses and says so: where the crossover would fall below the shortest the program has been checked at, it is held there (B-088, rules 630 to 637); and the entry breaks a body up on Eq. 12's I_f as the paper prints it, not on twice it as the program computes it, a departure named in I1 (B-089, rules 667 to 697).",
  "**An airburst's flash is where the airburst is, and it is too faint.** Since 21 September 2026 a complete airburst's burns and fires are drawn at the ground ranges its flash reaches from its burst altitude (rules 698 to 705, B-094); until then they were the flash's slant ranges drawn as if it were on the ground, and the Chelyabinsk preset drew third-degree burns within 1.3 km and a mass fire within 1 km of a flash 27 km up. The flash still carries Collins et al.'s luminous efficiency for the plume of an impact on the ground, 3 × 10⁻³, where superbolides radiate 3 to 20 % of their energy (Svetsov & Shuvalov 2018): so no preset airburst burns anything, where Tunguska started forest fires, scorched trees over 100 to 200 km² and burned exposed skin out to 50 km (Vasilyev 1998; Morrison 2018). That is the field's baseline, not this port's: NASA's risk model PAIR places an airburst's flash the same way, the ground range at which the slant distance to the burst is the flash's reach, with the same efficiency sampled from 10⁻⁴ to 10⁻² (Mathias et al. 2017). The field's airburst radiation models — NASA ATAP's (Johnston & Stern 2019), which gives Tunguska a tree burn about 10 km in radius, and IDG RAS's (Glazachev et al. 2020; Svetsov et al. 2025) — are not implemented here, because the tables that would verify them at an airburst's scale are not yet in hand (B-095).",
