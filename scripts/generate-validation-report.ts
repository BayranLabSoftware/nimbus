...
${bullet([
  ...
  "**An impact's air blast is read from under the ground, as the program reads it.** Since 16 September 2026 the blast of an impact that reaches the ground is the Earth Impact Effects Program's own (rules 138 to 145): its airburst law at the altitude Collins et al.'s Eq. 18 gives, which for such a body lies below the ground, so the crossover of the Mach relation shortens and the blast weakens as a steeper entry puts that altitude deeper. It reproduces the program within 1 % on every point read, and it is what the field's tool computes, not a physical source: no paper derives a burst below the surface. It has a price the invariants show — at a fixed range a larger body can blast less, and the impact family's sweep counts 407 blast rings that shrink as a body grows by 1 %, the program's own rings shrinking with them where it was asked. Where the program has no answer the simulator chooses and says so: where the crossover would be negative it draws no blast, and where the doubled I_f would keep a body whole it uses the paper's entry, which breaks Sikhote-Alin's body as the fall broke.",
