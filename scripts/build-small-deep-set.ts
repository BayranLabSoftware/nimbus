...
import type { SmallDeepEarthquake } from '../src/physics/validation/allenTollRules.js';
...
import { PAGER_PRODUCTS } from '../src/physics/validation/pagerProductsData.js';
...
/**
 * The earthquakes of rule 61 of `src/physics/validation/allenTollRules.ts`.
 *
 * Every record of the NCEI/WDS Global Significant Earthquake Database
 * dated 2008 to 2025 whose magnitude in the database is 4.0 to 4.99 and
 * whose focal depth is 40 km or less (`small`), or whose magnitude is 5 or
 * more and whose focal depth is more than 40 km (`deep`), matched to a USGS
 * ComCat event of magnitude 3.5 or more as rule 12 matches one — within
 ...
 * are one row; an event already in a set a rule has read, among the
 * campaign's PAGER products or its DART records is listed and left out.
 * Only those numbers are written. Nothing here runs the simulator.
 *
 * Usage:
 *   pnpm exec tsx scripts/build-small-deep-set.ts [cache directory]
 */

const NCEI_BASE =
  'https://www.ngdc.noaa.gov/hazel/hazard-service/api/v1/earthquakes?minYear=2008&maxYear=2025';
const NCEI_QUERIES: readonly { window: SmallDeepEarthquake['window']; query: string }[] = [
  { window: 'small', query: `${NCEI_BASE}&minEqMagnitude=4&maxEqMagnitude=4.99&maxEqDepth=40` },
  { window: 'deep', query: `${NCEI_BASE}&minEqMagnitude=5&minEqDepth=41&maxEqDepth=700` },
];
const COMCAT = 'https://earthquake.usgs.gov/fdsnws/event/1/query';
const MATCH_SECONDS = 120;
const MATCH_KM = 200;
const COMCAT_FLOOR = 3.5;
const CONCURRENCY = 2;
const CACHE = resolve(process.argv[2] ?? join(tmpdir(), 'nimbus-small-deep-set'));
...
async function nceiItems(): Promise<NceiItem[]> {
  ...
  for (const { window, query } of NCEI_QUERIES) {
    for (let page = 1; ; page++) {
      const d = (await json(`${query}&itemsPerPage=200&page=${page.toString()}`)) as {
        items: NceiItem[];
        totalPages: number;
      };
      items.push(...d.items.map((item) => ({ ...item, window })));
...
async function detail(
  ...
  SmallDeepEarthquake,
  'nceiIds' | 'date' | 'place' | 'deaths' | 'missing' | 'window'
> | null> {
  let d: Detail;
  try {
...
async function main(): Promise<void> {
  ...
  const rows: SmallDeepEarthquake[] = [];
  ...
  const line = (r: SmallDeepEarthquake): string =>
    `  [[${r.nceiIds.join(', ')}], ${JSON.stringify(r.date)}, ${JSON.stringify(r.place)}, ${JSON.stringify(r.comcat)}, ${JSON.stringify(r.time)}, ${r.magnitude.toString()}, ${JSON.stringify(r.magnitudeType)}, ${r.depthKm.toString()}, ${r.latitude.toString()}, ${r.longitude.toString()}, ${JSON.stringify(r.faultType)}, ${r.deaths.toString()}, ${r.missing.toString()}, ${JSON.stringify(r.window)}],`;
  const body = `// Generated by scripts/build-small-deep-set.ts on ${readOn}. Do not edit
 ...
 *  window, by NCEI id. */
export const SMALL_DEEP_UNMATCHED: readonly number[] = ${JSON.stringify(unmatched)};

/** Events already read — in a rule's set, a PAGER product or a DART
 *  record — left out. */
export const SMALL_DEEP_EXCLUDED: readonly string[] = ${JSON.stringify(excluded)};

/** Events ComCat would not serve that day, left out. */
export const SMALL_DEEP_UNREADABLE: readonly string[] = ${JSON.stringify(gone)};
`;
  const out = join(
    resolve(dirname(fileURLToPath(import.meta.url)), '..'),
    'src',
