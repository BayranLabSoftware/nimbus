...
/**
 * The earthquakes of rule 50 of `src/physics/validation/pointSourceRules.ts`.
 *
 * Every M 6 or larger event of 2000 to 2007, no deeper than 40 km, that USGS
 ...
 *
 * Usage:
 *   pnpm exec tsx scripts/build-point-source-set.ts [cache directory]
 *
...
const LIST = `${COMCAT}?format=geojson&starttime=2000-01-01&endtime=2008-01-01&minmagnitude=6&maxdepth=40&producttype=shakemap&orderby=time-asc`;
const NCEI =
  'https://www.ngdc.noaa.gov/hazel/hazard-service/api/v1/earthquakes?minYear=2000&maxYear=2007';
const COVERAGE = 'download/coverage_mmi_low_res.covjson';
const INFO = 'download/info.json';
const CONCURRENCY = 2;
const CACHE = resolve(process.argv[2] ?? join(tmpdir(), 'nimbus-point-source-set'));
...
function rowText(r: PointSourceEarthquake): string {
...
async function main(): Promise<void> {
  ...
  const rows = new Map<string, PointSourceEarthquake>();
  ...
  const body = `// Generated by scripts/build-point-source-set.ts on ${readOn}. Do not edit
// by hand: rule 50 of pointSourceRules.ts says what these are and how they
// were read. USGS ComCat and ShakeMap (${COVERAGE}, ${INFO}), public domain;
...
export const POINT_SOURCE_WITHOUT_COVERAGE: readonly string[] = ${JSON.stringify(missing)};

/** Listed events whose ShakeMap carries no model weights, left out. */
export const POINT_SOURCE_WITHOUT_MODEL: readonly string[] = ${JSON.stringify(noModel)};

/** Listed events ComCat would not serve that day, left out. */
export const POINT_SOURCE_UNREADABLE: readonly string[] = ${JSON.stringify(gone)};
`;
  const out = join(
